package ani.saikou.app.android.adapter.manga
...
import ani.saikou.app.databinding.ItemImageBinding
import ani.saikou.app.util.px
import ani.saikou.core.model.manga.MangaChapter
import ani.saikou.core.model.settings.CurrentReaderSettings
...
class ImageAdapter(
    ...
    private val images = chapter.images ?: arrayListOf()
    ...
    fun loadImage(imageView: SubsamplingScaleImageView, position: Int, parent: View) {
        ...
        Glide.with(imageView)
            .download(GlideUrl(images[position]) { chapter.headers ?: mutableMapOf() })
...
//                        toastString("Failed to load Page ${position+1}, Long Click to reload")
                    ...
                    override fun onResourceReady(resource: File, transition: Transition<in File>?) {
                        ...
                        ObjectAnimator.ofFloat(parent, "alpha", 0f, 1f)
                            .setDuration((400 * uiSettings.animationSpeed).toLong()).start()
                        progress.visibility = View.GONE
                    }
                }
