package ani.saikou.app.android.activity.manga
...
import ani.saikou.core.source.manga.HMangaSources
...
class MangaReaderActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        settings = STORE.loadData("reader_settings") ?: ReaderSettings().apply {
            STORE.saveData(
                "reader_settings",
                this
            )
        }
        uiSettings = STORE.loadData("ui_settings") ?: UserInterfaceSettings().apply {
            STORE.saveData(
                "ui_settings",
                this
            )
                ...
                if (isContVisible) {
                    ...
                    if (!isAnimating) {
                        ...
                        ObjectAnimator.ofFloat(
                            binding.mangaReaderBottomCont,
                            "translationY",
                            0f,
                            128f
                        ).apply { interpolator = overshoot;duration = controllerDuration;start() }
                        ObjectAnimator.ofFloat(
                            binding.mangaReaderTopCont,
                            "translationY",
                            0f,
                            -128f
                        ).apply { interpolator = overshoot;duration = controllerDuration;start() }
        ...
        binding.mangaReaderRecycler.longTapListener = { event ->
            binding.mangaReaderRecycler.findChildViewUnder(event.x, event.y).let { child ->
                val image =
                    child?.findViewById<SubsamplingScaleImageView>(R.id.imgProgImageNoGestures)
                if (image != null) {
                    adapter?.loadImage(
                        image,
        ...
        showProgressDialog =
            if (settings.askIndividual) STORE.loadData<Boolean>("${media.id}_progressDialog") != true else false
        progressDialog =
            if (showProgressDialog && Anilist.userid != null && if (media.isAdult) settings.updateForH else true) AlertDialog.Builder(
                ...
                setMultiChoiceItems(
                    ...
                    if (isChecked) {
                        STORE.saveData("${media.id}_progressDialog", isChecked)
                        progressDialog = null
                    }
