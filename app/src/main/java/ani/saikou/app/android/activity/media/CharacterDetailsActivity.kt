...
import ani.saikou.app.android.adapter.media.CharacterDetailsAdapter
import ani.saikou.app.android.adapter.media.MediaAdapter
import ani.saikou.app.android.model.media.OtherDetailsViewModel
import ani.saikou.app.databinding.ActivityCharacterBinding
...
class CharacterDetailsActivity : AppCompatActivity(), AppBarLayout.OnOffsetChangedListener {
    ...
    val uiSettings = STORE.loadData<UserInterfaceSettings>("ui_settings") ?: UserInterfaceSettings()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val banner =
            if (uiSettings.bannerAnimations)
                binding.characterBanner
            else
                binding.characterBannerNoKen
        ...
        model.getCharacter().observe(this) {
            if (it != null && !loaded) {
                ...
                if (roles != null) {
                    val mediaAdaptor = MediaAdapter(0, roles, this, matchParent = true)
                    val concatAdaptor =
    ...
    override fun onOffsetChanged(appBar: AppBarLayout, i: Int) {
        ...
        binding.characterCover.visibility =
            if (binding.characterCover.scaleX == 0f)
                View.GONE
            else
                View.VISIBLE
