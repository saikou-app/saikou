package ani.saikou.app.util.manga.zoomrecycler
...
open class ZoomRecyclerView @JvmOverloads constructor(
    ...
    private var currentScale = defaultZoom
    ...
    private fun zoom(
        ...
        animatorSet.duration = zoomDurationTime.toLong()
    ...
    fun onScaleEnd() {
        if (scaleX < minZoom) {
            zoom(currentScale, minZoom, x, 0f, y, 0f)
        }
    }
    ...
    inner class GestureListener : GestureDetectorWithLongTap.Listener() {
        ...
        fun onDoubleTapConfirmed(ev: MotionEvent) {
            if (!isZooming) {
                if (scaleX != defaultZoom) {
                    zoom(currentScale, defaultZoom, x, 0f, y, 0f)
                } else {
                    val toScale = 2f
                    val toX = (halfWidth - ev.x) * (toScale - 1)
                    val toY = (halfHeight - ev.y) * (toScale - 1)
                    zoom(defaultZoom, toScale, 0f, toX, 0f, toY)
                }
            }
        }
