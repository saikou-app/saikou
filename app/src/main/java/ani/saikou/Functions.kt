...
fun getSize(url: String, headers: MutableMap<String, String>? = null): Long? {
    return try {
        Jsoup.connect(url)
            ...
            .execute().header("Content-Length")?.toLong()?.div(1048576)
    } catch (e: Exception) {
        //        logger(e)
        null
    }
...
fun download(activity: Activity, episode: Episode, animeTitle: String) {
    ...
    val uri =
        if (stream.videoQuality.size > episode.selectedQuality) Uri.parse(stream.videoQuality[episode.selectedQuality].videoUrl) else return
    val regex = "[\\\\/:*?\"<>|]".toRegex()
    ...
    CoroutineScope(Dispatchers.IO).launch {
        try {
            ...
            if (loadData<Boolean>("sd_dl") == true && arrayOfFiles.size > 1 && arrayOfFiles[0] != null && arrayOfFiles[1] != null) {
                ...
                request.setDestinationUri(Uri.fromFile(File("$parentDirectory$title (${stream.videoQuality[episode.selectedQuality].quality}).mp4")))
            } else {
                val direct = File(Environment.DIRECTORY_DOWNLOADS + "/Saikou/Anime/${aTitle}/")
                if (!direct.exists()) direct.mkdirs()
                request.setDestinationInExternalPublicDir(
                    Environment.DIRECTORY_DOWNLOADS,
                    "/Saikou/Anime/${aTitle}/$title (${stream.videoQuality[episode.selectedQuality].quality}).mp4"
                )
            }
...
open class Registry<T: Named> {
     ...
     * Underlying registry.
     *
     * Maps the provider's name ([String]) to the provider itself ([IMediaProvider]).
     */
