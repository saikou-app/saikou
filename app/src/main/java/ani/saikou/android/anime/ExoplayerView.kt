...
import ani.saikou.android.anime.source.AnimeSources
import ani.saikou.android.anime.source.HAnimeSources
import ani.saikou.android.databinding.ActivityExoplayerBinding
...
class ExoplayerView : AppCompatActivity(), Player.Listener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //Speed
        val speeds = if (settings.cursedSpeeds) arrayOf(
            1f,
            1.25f,
            1.5f,
            1.75f,
            ...
            25f,
            50f
        ) else arrayOf(0.25f, 0.33f, 0.5f, 0.66f, 0.75f, 1f, 1.25f, 1.33f, 1.5f, 1.66f, 1.75f, 2f)
        ...
        if (showProgressDialog && Anilist.userid != null && if (media.isAdult) settings.updateForH else true)
            AlertDialog.Builder(this, R.style.DialogTheme)
                .setTitle("Auto Update progress for ${media.userPreferredName}?").apply {
...
//                }
    ...
    private fun initPlayer() {
        ...
        val url =
            if (episode.selectedQuality < stream.quality.size) stream.quality[episode.selectedQuality] else return
        ...
        if (playbackPosition != 0L && !changingServer && !settings.alwaysContinue) {
            ...
            AlertDialog.Builder(this, R.style.DialogTheme).setTitle("Continue from ${time}?")
                .apply {
                    setCancelable(false)
...
//        exoPlayer.playbackParameters = playbackParameters
    ...
    private fun cast() {
        ...
        stream.subtitles?.get("English")?.let { shareVideo.putExtra("subtitle", it) }
