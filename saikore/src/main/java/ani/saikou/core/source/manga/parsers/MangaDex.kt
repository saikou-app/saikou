...
class MangaDex(override val name: String = "mangadex.org") : MangaParser() {
    ...
    override fun getLinkChapters(link: String): MutableMap<String, MangaChapter> {
        ...
        try {
            ...
            (0..totalChapters step 200).reversed().forEach { index ->
                ...
                Json.decodeFromString<JsonObject>(jsonResponse)["data"]!!.jsonArray.reversed()
                    .forEach {
                        if (it.jsonObject["attributes"]!!.jsonObject["translatedLanguage"].toString() == "\"en\"") {
                            val chapter =
                                it.jsonObject["attributes"]!!.jsonObject["chapter"].toString()
                                    .trim('"')
                            val title = it.jsonObject["attributes"]!!.jsonObject["title"].toString()
                                .trim('"')
    ...
    override fun getChapter(chapter: MangaChapter): MangaChapter {
        try {
            val json = Json.decodeFromString<JsonObject>(
                Jsoup.connect("$host/at-home/server/${chapter.link}").ignoreContentType(true).get()
                    .text()
            )
    ...
    override fun search(string: String): ArrayList<Source> {
        ...
        try {
            val jsonResponse =
                Jsoup.connect("$host/manga?limit=$limit&title=$string&order[relevance]=desc&includes[]=cover_art")
                    .ignoreContentType(true).get().text()
            Json.decodeFromString<JsonObject>(jsonResponse)["data"]!!.jsonArray.forEach {
                val id = it.jsonObject["id"].toString().trim('"') // id
                val title =
                    it.jsonObject["attributes"]!!.jsonObject["title"]!!.jsonObject["en"].toString()
                        .trim('"') // en title
                val coverName =
    ...
    override fun saveSource(source: Source, id: Int, selected: Boolean) {
        ...
        STORE.saveData("mangadex_$id", source)
    }
}